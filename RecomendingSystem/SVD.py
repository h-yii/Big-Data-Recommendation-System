...
class SVDModel(object):
    ...
    def loadTrainSet(self):
        ...
        self.overall_train_mean_rating=0.0 #TODO 这个如果用划分好的训练集来训练 就是训练集的评分还是？整个稀疏矩阵的平均分？？？
    ...
    # 对rp进行边界检查（须在0到100之间）    rp = float(max(self.rating_scale[0], min(int(rp), self.rating_scale[1])))
    ...
    def train(self): #TODO：这里进行 降重！ 把P参数和Q参数改成用np库的 进行降重——其实维度 只要进行一个转置就行——TO TRY IT！
        print("开始训练...")
        ...
        for epoch in range(0, self.epochs):
            ...
            rmse /= self.num_trainingData #TODO 由于训练集每次都条数一样 所以这里其实可以去掉！！！
            rmse = math.sqrt(rmse)
...
# 【ok】
    def loadItemAttributeDataset(self):
        with open('./data/processed_itemAttribute.csv', 'r') as f:
    ...
    def linear(self):
        ...
        for k, v in self.user_item_attrs.items():
            ...
            # param = self.basic_linear(k)  # 对每个用户，用最小二乘法拟合残差值
            # self.user_para[k] = param  # 存储每个用户对应的参数a, b, c
...
# 【ok】
    def predictOnTestDataset(self):#在test.txt上进行预测，得到本次实验结果
        ...
        res1 = open(RESULT_FOLDER+'result1.txt', 'w')  # 优化前
